...
def generate_correlation_id(source_message: func.ServiceBusMessage) -> str:
    ...
    """
    if source_message.correlation_id:
        return source_message.correlation_id
    
    timestamp = datetime.datetime.utcnow().isoformat()
    return f"{CORRELATION_ID_PREFIX}-{timestamp}"


def process_message_body(source_body: Any, original_content_type: str | None) -> tuple[bytes, str]:
    """
...
def generate_replicated_message_id(correlation_id: str, original_message_id: str | None) -> str:
    """
    ...
    """
    correlation_prefix = correlation_id[:8] if len(correlation_id) >= 8 else correlation_id
...
def create_replicated_message(
    ...
    processed_body, final_content_type = process_message_body(source_body, original_content_type)
    ...
    new_message_id = generate_replicated_message_id(correlation_id, source_message.message_id)
    ...
    return ServiceBusMessage(
        ...
        scheduled_enqueue_time_utc=getattr(source_message, "scheduled_enqueue_time_utc", None),
