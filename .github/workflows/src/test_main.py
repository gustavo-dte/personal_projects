...
from .config import ReplicationConfig, RetryConfig, DeadLetterConfig
...
class TestErrorHandling:
    ...
    def test_handle_authentication_error(self):
        ...
        with pytest.raises(ClientAuthenticationError) as exc_info:
            handle_authentication_error(error, correlation_id, direction, destination_queue)
...
class TestReplicationOrchestration:
    ...
    def test_orchestrate_replication_success(self, mock_replicate):
        ...
        mock_config.get_destination_config.return_value = (
            "conn_str", "queue_name", "Primary → Secondary"
        )
    ...
    def test_orchestrate_replication_validation_error(self):
        ...
        mock_message = Mock()
        mock_config = Mock()
        mock_config.get_destination_config.side_effect = ValidationError("Invalid config")
...
class TestIntegration:
    ...
    def test_send_message_to_destination_integration(self, mock_create_msg, mock_client_class):
        """Test the complete message sending flow."""
        # Setup mocks
        mock_client = Mock()
        mock_sender = Mock()
        mock_client.get_queue_sender.return_value.__enter__.return_value = mock_sender
        mock_client_class.from_connection_string.return_value.__enter__.return_value = mock_client
        ...
        # Verify
        mock_client_class.from_connection_string.assert_called_once_with("test_conn_str")
