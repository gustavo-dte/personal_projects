...
def replicate_message(
    *,
    ...
    correlation_id = (
        source_message.correlation_id or
        f"repl-{datetime.datetime.utcnow().isoformat()}"
    )
    
    app_logger.debug("Starting message replication", extra={
    ...
    try:
        ...
        with ServiceBusClient.from_connection_string(
            ...
            with queue_sender:
                ...
                preserved_app_properties = getattr(
                    source_message, "application_properties", {}
                ) or {}
                enhanced_app_properties = preserved_app_properties.copy()
                enhanced_app_properties["x-original-message-id"] = (
                    source_message.message_id
                )
                enhanced_app_properties["x-replication-correlation-id"] = (
                    correlation_id
                )
                ...
                replicated_message = ServiceBusMessage(
                    ...
                    message_id=(
                        ...
                        if source_message.message_id else f"repl-{correlation_id[:8]}"
                    )
                )
                ...
                app_logger.info("Message replication successful", extra={
                    "correlation_id": correlation_id,
                    "direction": direction,
                    "destination_queue": destination_queue_name,
                    ...
                    "content_type": final_content_type,
                    "body_size_bytes": len(replicated_body) if replicated_body else 0
                })

    except ClientAuthenticationError as auth_error:
        # Authentication failures are critical and should trigger immediate alerts
        app_logger.error("Service Bus authentication failed", extra={
            "correlation_id": correlation_id,
        ...
        app_logger.error("Service Bus resource not found", extra={
            "correlation_id": correlation_id,
        ...
        app_logger.error("Service Bus request failed", extra={
            "correlation_id": correlation_id,
        ...
        app_logger.error("Service Bus operation failed", extra={
            "correlation_id": correlation_id,
        ...
        raise ServiceBusError(
            f"Service Bus operation failed: {sb_error}"
        ) from sb_error

    except HttpResponseError as http_error:
        # HTTP-level errors from Azure services
        app_logger.error("HTTP request failed during replication", extra={
            "correlation_id": correlation_id,
        ...
        app_logger.error("Azure service error during replication", extra={
            "correlation_id": correlation_id,
        ...
        app_logger.error("Unexpected error during replication", extra={
            "correlation_id": correlation_id,
...
def load_config() -> ReplicationConfig:
    ...
    try:
        ...
        app_logger.info("Replication configuration loaded successfully", extra={
        ...
        app_logger.error("Configuration validation failed", extra={
...
def replicate(
    *,
    ...
    app_logger.info("Starting message replication process", extra={
