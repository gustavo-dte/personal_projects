#! /usr/bin/env pwsh
# PowerShell script to configure Dynatrace extension on Windows VMs
...
try {
  Write-Output "INFO: Configuring Dynatrace extension for Windows VM '$VMName'..."
  ...
  Write-Output "INFO: VM PowerState: $(($VM.PowerState -split '/')[-1])"
