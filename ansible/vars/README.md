...
# Domain join configuration (required for join-windows-domain.yaml playbook)
domainNameSetup:
  domain: "example.com"  # Required: FQDN of the Active Directory domain
  domain_admin_username: "admin@example.com"  # Required: Domain admin username (UPN format)
  domain_ou_path: "OU=Computers,DC=example,DC=com"  # Optional: OU path for computer account placement
...
- Keep secrets out of manifests; these files are intended to be committed and passed to Ansible with `-e @manifest.yml`.
  - **Domain credentials**: For production use, store `domain_admin_password` in Azure Key Vault or Ansible Vault, not in the manifest file.
