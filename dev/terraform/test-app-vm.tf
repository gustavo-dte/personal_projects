...
# NOTE: Terraform import blocks don't support for_each directly.
# For each new VM added to local.test_app_vms, you need to add import blocks following this pattern:
...
import {
  ...
  id = local.test_app_vms["vmcuwinwebd01"].nic_resource_id
}

import {
  to = module.test_app_vms["vmcuwinwebd01"].azurerm_managed_disk.os_disk
  id = local.test_app_vms["vmcuwinwebd01"].os_disk_resource_id
}
...
import {
  ...
  id = local.test_app_vms["vmcuwinwebd01"].disk.data["vmcuwinwebd01-datadisk-01-073cfec7-5b88-491e-8e62-3b6b95371acc"].resource_id
}
...
import {
  ...
  id = "${local.test_app_vms["vmcuwinwebd01"].vm_resource_id}/dataDisks/vmcuwinwebd01-datadisk-01-073cfec7-5b88-491e-8e62-3b6b95371acc"
}

import {
  to = module.test_app_vms["vmcuwinwebd01"].azurerm_windows_virtual_machine_scale_set.vmss[0]
  id = "${local.test_app_resource_prefix}/providers/Microsoft.Compute/virtualMachineScaleSets/${local.test_app_vms["vmcuwinwebd01"].vmss_name}"
}
